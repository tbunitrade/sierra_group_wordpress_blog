...
function process_postviews() {
	...
	if ( ! wp_is_post_revision( $post ) && ! is_preview() ) {
		if ( is_single() || is_page() ) {
			...
			if ( !$post_views = get_post_meta( $post->ID, 'просм.', true ) ) {
				$post_views = 0;
			}
			$should_count = false;
...
function views_activation( $network_wide ) {
	...
	$option = array(
		...
		'use_ajax' => 1,
		//%VIEW_COUNT% views
		'template' => __( '%VIEW_COUNT% 123', 'wp-postviews' ),
		'most_viewed_template' => '<li><a href="%POST_URL%"  title="%POST_TITLE%">%POST_TITLE%</a> - %VIEW_COUNT% '.__('123 ', 'wp-postviews').'</li>'
	);
